...
def _import_webapp():
    ...
    try:
        ...
        raise RuntimeError(
            "boost_sales.api.server must export either `create_app(...)` or `app`."
        ) from e
...
def cmd_train(
    ...
    hol_subdiv: T.Optional[str] = typer.Option(None, help="Holidays subdivision."),
    horizons: T.Optional[str] = typer.Option(None, help="Horizons like '1-7' or '1,2,3'."),
    nthread: int = typer.Option(1, help="XGBoost nthread parameter."),
...
def cmd_train_per_group(
    scope: str = typer.Argument(..., help="Scope: 'pair' (store+item), 'item', or 'store'."),
    data_csv: Path = typer.Option(..., help="Path to sales CSV."),
    ...
    nthread: int = typer.Option(1, help="XGBoost nthread parameter."),
    verbose_eval: int = typer.Option(0, help="XGBoost training verbosity (0/1)."),
    enforce_single_thread_env: bool = typer.Option(False, help="Set single-thread env vars to 1."),
):
...
def cmd_forecast(
    ...
    latest_only: bool = typer.Option(
        False, help="Keep only the latest base date per (store,item) before scoring."
    ),
    hol_country: str = typer.Option("US", help="Holidays country code to use."),
    hol_subdiv: T.Optional[str] = typer.Option(None, help="Holidays subdivision."),
    horizons: T.Optional[str] = typer.Option(None, help="Horizons to produce (e.g. '1-7')."),
    assume_no_change: bool = typer.Option(
        False, help="Fill price/promo_h{h} from current values and compute price_ratio_h{h} safely."
    ),
):
    cfg = AppConfig()
    cfg.paths.data_csv = data_csv
    ...
    preds = pd.concat(all_preds, ignore_index=True).sort_values(
        ["store_id", "item_id", "target_date", "horizon"]
    )
...
def cmd_serve_web(
    models_dir: Path = typer.Option(..., help="Directory with trained models."),
    data_csv: T.Optional[Path] = typer.Option(None, help="Optional demo CSV path."),
    hol_country: str = typer.Option("US", help="Holidays country code to use."),
    hol_subdiv: T.Optional[str] = typer.Option(None, help="Holidays subdivision."),
    horizons: T.Optional[str] = typer.Option(None, help="Horizons exposed by the service."),
    assume_no_change: bool = typer.Option(True, help="Default assume-no-change behavior."),
